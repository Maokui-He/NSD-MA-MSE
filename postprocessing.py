...
def write_rttm(session_label, output_path, min_segments):
...
def main(args):
    ...
    for p in prob_array:
        ...
        num_speaker, T, C = prob_label.shape
        session_label[session] = np.zeros([num_speaker, T])
        #session_label[session][prob_label[..., 1] > args.threshold / 100] = 1
        session_label[session][prob_label[..., 1] > args.threshold / 100] = 1
        #silence = np.sum(session_label[session], axis=0)
        #session_label[session][ (silence==0)[None, :] & (prob_label[..., 1] > 0.5) ] = 1
    ...
    write_rttm(session_label, os.path.join(args.prob_array_dir, "rttm_th{:.2f}".format(args.threshold/100)), args.min_segments)
