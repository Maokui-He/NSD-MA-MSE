...
def collate_fn_mask_all_one(batch):
    '''
    batch: B * (data, embedding, label)
    '''
    num_speaker = batch[0][1].shape[0]
...
def collate_fn_mask_random_assign_overlap(batch):
    '''
    batch: B * (data, embedding, label)
    '''
    num_speaker = batch[0][1].shape[0]
    ...
    for i, id in enumerate(ordered_index):
        ...
        input_data[i, :length[id], :] = batch[id][0]
        speaker_embedding.append(batch[id][1][speaker_index])
        mask = copy.deepcopy(batch[id][2][..., 1])
        #mask = mask.asdtype(np.float32)
        #overlap = np.sum(mask, axis=0)
        sort_index = np.argsort(-mask, axis=0)
        #random_index = np.floor(np.random.uniform(0, 1, length[id]) * overlap).astype(int)
        #select_index = sort_index[random_index, range(length[id])]
        zero_mask = np.zeros(mask.shape)
        #zero_mask[select_index, range(length[id])] = 1
        zero_mask[sort_index[0, :], range(length[id])] = 1
        if np.sum(zero_mask) != length[id]: print(np.sum(zero_mask))
        #mask[:, overlap>1] = 0
        mask_data[i, :, :length[id]] = mask[speaker_index] * zero_mask[speaker_index]
        #print(batch[id][2].shape)
        label_data.append(torch.from_numpy(batch[id][2][speaker_index].astype(np.float32)))
        nframes.append(length[id])
    input_data = torch.from_numpy(input_data).transpose(1, 2) # B * T * F => B * F * T
...
class RTTM_to_Speaker_Mask():
    ...
    def get_mixture_utternce_label(self, session, speaker_embedding, start=0, end=None):
        speakers = []
